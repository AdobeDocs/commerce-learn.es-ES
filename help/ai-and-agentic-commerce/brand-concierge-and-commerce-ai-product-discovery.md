---
title: Descubrimiento de productos de IA de Brand Concierge y Commerce
description: Descubra cómo Brand Concierge utiliza un agente de catálogo para cerrar las lagunas de visibilidad de la IA, lo que enriquece los datos de productos de Commerce para que los LLM puedan descubrir y recomendar sus productos.
role: User
level: Beginner
doc-type: Value Video
last-substantial-update: 2026-06-11T00:00:00Z
jira: KT-21572
duration: 451
source-git-commit: 3475138b5a0dd2eaa30ee7da0e3f7b2699428d59
workflow-type: tm+mt
source-wordcount: '180'
ht-degree: 0%

---

# Descubrimiento de productos de IA de Brand Concierge y Commerce

[!BADGE Próximamente]{type=Informative tooltip="Esta función estará disponible próximamente y sujeta a cambios."}

Incluso los catálogos de productos más ricos tienen solo una visibilidad parcial de los modelos de lenguaje grande (LLM). Los atributos enriquecidos como las variantes de producto, el tipo de molienda, las notas de sabor y el empaquetado suelen ocultarse detrás de paneles y ventanas emergentes contraíbles que los rastreadores de IA no pueden leer. El agente de catálogo comercial identifica automáticamente estas lagunas de visibilidad y las resuelve, sin cambiar la tienda orientada al comprador.

## ¿Para quién es este vídeo?

* Comerciantes
* Administradores de Commerce
* Especialistas en marketing digital

## Contenido de vídeo

* Comprobador de visibilidad de IA para simular cómo leen los rastreadores LLM las páginas de producto
* El panel Oportunidades de optimización presenta recomendaciones de enriquecimiento de alto impacto
* Enriquecimiento de la página de detalles del producto para exponer los atributos ocultos a los rastreadores LLM
* Enriquecimiento del catálogo que genera nombres y descripciones de productos optimizados para IA
* Validación de mejoras mediante el Comprobador de Visibilidades del contenido de IA

>[!VIDEO](https://video.tv.adobe.com/v/3492287)

## Recursos adicionales

[Documentación de [!DNL Brand Concierge]](https://experienceleague.adobe.com/en/docs/brand-concierge/content/home){target="_blank"}

